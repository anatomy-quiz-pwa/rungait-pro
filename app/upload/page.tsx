...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (presignRes.status === 401) throw new Error("登入已失效，請重新登入")
      if (presignRes.status === 403) throw new Error("此帳號尚未開通上傳權限")
      if (!presignRes.ok) throw new Error("預簽失敗（" + presignRes.status + "）")
      ...
      setMessage("錯誤：" + (err instanceof Error ? err.message : String(err)))
    } finally {
      setUploading(false)
    }
  }
    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-black flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white/10 dark:bg-zinc-900 border border-zinc-700 rounded-2xl p-6 space-y-4 shadow-lg">
        ...
        {videoUrl && (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setStartTime(videoRef.current?.currentTime ?? null)}
                className="px-3 py-2 bg-emerald-600 text-white text-sm rounded"
              >
                設為起點
              </button>
              <button
                onClick={() => setEndTime(videoRef.current?.currentTime ?? null)}
                className="px-3 py-2 bg-emerald-600 text-white text-sm rounded"
              >
                設為終點
              </button>
        ...
        <div className="text-right">
          <Link href="/single" className="text-xs text-zinc-400 hover:text-blue-400">
            查看分析清單
          </Link>
        </div>
      </div>
